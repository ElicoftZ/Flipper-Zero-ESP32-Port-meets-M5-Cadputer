...
BOARD="waveshare_c6_1.47"
BUILD_DIR="build_waveshare_c6_1.47"
IDF_TARGET="esp32c6"

# The board-specific sdkconfig fragment is named after the board, NOT the IDF
# target (esp32c6), so ESP-IDF won't pick it up automatically — pass it
# explicitly. Keep sdkconfig inside the build dir so this board doesn't clobber
# the git-tracked root sdkconfig (which the T-Embed/esp32s3 build uses).
...
ESP32-C6-LCD-1.47 board (single-app, 4MB flash, no multi-boot).
